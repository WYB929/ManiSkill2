...
@register_env("Real_Table-v1", max_episode_steps=200)
class StackCubeEnv(StationaryManipulationEnv):
    ...
    def _load_actors(self):
        ...
        self.table = self._build_table(size=0.8, height=self.table_height, thickness=self.table_thickness)
        ...
        # boxA_urdf = sp.asset.download_partnet_mobility(100174, token=SAPIEN_ACCESS_TOKEN)
        urdf_loader = self._scene.create_urdf_loader()
        urdf_loader.fix_root_link = False
        # self.boxA = urdf_loader.load(boxA_urdf)
    ...
    def _initialize_actors(self):
        ...
        bottleA_xy = xy + sampler.sample(radius, 100, verbose=False)
        bottleB_xy = xy + sampler.sample(radius, 100, verbose=False)
        ...
        self.bottleA.set_pose(sapien.Pose([bottleA_xy[0], bottleA_xy[1], self.table_height+0.05]))
        self.bottleB.set_pose(sapien.Pose([bottleB_xy[0], bottleB_xy[1], self.table_height+0.05]))
    ...
    def evaluate(self, **kwargs):
        ...
        success = is_cubeA_on_cubeB and is_cubeB_on_cubeC and is_cubeA_static and is_cubeB_static and (not is_cubaA_grasped) and (not is_cubeB_grasped)
