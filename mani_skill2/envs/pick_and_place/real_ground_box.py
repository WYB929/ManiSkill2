...
from mani_skill2.utils.sapien_utils import set_actor_visibility, vectorize_pose, check_urdf_config, parse_urdf_config
...
class PickSingleEnv(StationaryManipulationEnv):
    ...
    def _load_actors(self):
        ...
        self.box.set_pose(Pose([0, -0.3, 0.5], [1, 0, 0, -1]))
...
class PickSingleYCBEnv(PickSingleEnv):
    ...
    def _load_model(self):
        self._set_model("002_master_chef_can", 0.5)
        ...
        # self.obj.set_pose(sapien.Pose([0.2, 0.2, 0]))

        box_urdf = sp.asset.download_partnet_mobility(100174, token=SAPIEN_ACCESS_TOKEN)
        loader = self._scene.create_urdf_loader()
        loader.fix_root_link = False
        loader.scale = 0.2
        loader.load_multiple_collisions_from_file = True
        self.box = loader.load(box_urdf)
        # self.box.set_pose(sapien.Pose([-0.1, -0.1, 0.0]))
